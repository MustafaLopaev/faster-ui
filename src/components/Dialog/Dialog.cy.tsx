...
// Where keyboard focus currently lives. Native modal dialogs let sequential
// navigation pass through the browser chrome (activeElement = body for one
// stop) — the trap guarantee is that focus NEVER reaches inert background
// content and always re-enters the dialog.
...
describe('<Dialog /> top layer & inert background (D2)', () => {
  ...
  it('makes the background inert to pointer and keyboard', () => {
    ...
    cy.get('[data-cy=background]').then(($btn) => {
      ...
      cy.get('[data-cy=trigger]').realClick()
      cy.get('dialog').should('have.attr', 'open')
      // Real CDP click at the background button's coordinates hits the top layer instead
      cy.get('body').realClick({ x: Math.round(rect.x + rect.width / 2), y: Math.round(rect.y + rect.height / 2) })
      cy.get('@background').should('not.have.been.called')
    })
    // Keyboard: repeated Tab never lands on inert background controls
    for (let i = 0; i < 6; i += 1) {
      cy.realPress('Tab')
      cy.document().should((doc) => {
...
describe('<Dialog /> focus trap across full Tab cycles (D5)', () => {
  it('keeps Tab and Shift+Tab cycling within the dialog focusables', () => {
    ...
    cy.get('[data-cy=trigger]').realClick()
    cy.get('dialog').should('have.attr', 'open')
    // Full forward traversal in document order: close → Cancel → Confirm
    ...
    // …and back again with Shift+Tab
    cy.realPress(['Shift', 'Tab'])
    cy.contains('dialog button', 'Cancel').should('have.focus')
    cy.realPress(['Shift', 'Tab'])
    cy.get('dialog button[aria-label=Close]').should('have.focus')
    // Tab past either end may pass through the browser chrome (native modal
    // semantics) but must NEVER land on inert page content
    cy.realPress(['Shift', 'Tab'])
    cy.document().should((doc) => {
      expect(focusLocation(doc), 'backward past the first focusable').to.be.oneOf(['dialog', 'chrome'])
    })
    cy.mount(<Harness />)
    cy.get('[data-cy=trigger]').realClick()
    ...
    cy.realPress('Tab')
    cy.document().should((doc) => {
      expect(focusLocation(doc), 'forward past the last focusable').to.be.oneOf(['dialog', 'chrome'])
    })
  })
