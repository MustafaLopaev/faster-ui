...
/**
 * Button's style configuration — every class string the component can apply.
 *
 * Kept out of `Button.tsx` so the component file reads as behaviour and this
 * one reads as the Figma matrix it transcribes. No variant library: these are
 * plain lookup maps of static `fui:*` strings joined by `cn()` (002 research
 * R-1), which is what keeps them statically analysable by Tailwind and by
 * `npm run lint:tokens`.
 *
 * Keys are the const-object members rather than bare strings, so a renamed
 * variant is a compile error here instead of a silently missing style.
 */
...
export const BUTTON_SIZE: Record<ButtonSize, string> = {
  [ButtonSize.lg]: 'fui:h-10 fui:px-2 fui:py-2 fui:min-w-26.5 fui:text-subtitle fui:rounded-control',
  [ButtonSize.md]: 'fui:h-9 fui:px-2 fui:py-1.75 fui:min-w-24.5 fui:text-body fui:rounded-control',
  [ButtonSize.sm]: 'fui:h-6 fui:px-1 fui:py-0.75 fui:min-w-15.5 fui:text-caption fui:rounded-control',
}
...
/**
 * Circular sets: outline tints its FILL on hover/active while the border stays
 * neutral — deliberately different from the text outline set (§1a).
 */
