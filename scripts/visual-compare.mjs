 ...
 *
 * COLD START — `visual/baselines/` is empty — is its own outcome, not a run in
 * which every cell happens to be `new`. Nothing has been established, so this
 * check neither passes nor fails: it says so, loudly, and exits 0. The same
 * reasoning the drift watcher applies to `unreachable` (FR-032) — an all-clear
 * a check did not establish is worse than no report, and a failure it cannot
 * substantiate is just noise that blocks every pull request until someone mutes
 * it. Judgment is skipped too: characterising 239 cells against nothing to
 * compare them to costs real money and produces no information.
 */
...
if (baselines.length === 0) {
  writeFileSync(
    ...
    JSON.stringify(
      {
        coldStart: true,
        counts: { total: current.length, unchanged: 0, changed: 0, new: current.length, orphaned: 0 },
        toJudge: [],
        results: current.map((cell) => ({ cell, state: 'new', diffRatio: null, judged: false })),
      },
      null,
      2,
    ) + '\n',
  ...
  const message =
    `No baseline set: visual/baselines/ holds no images, so ${current.length} captured cells ` +
    'have nothing to be compared against. This check did not run — it is neither a pass nor ' +
    'a failure. Generate baselines on the runner with: gh workflow run visual.yml -f accept-baselines=true'

  console.log(`Visual comparison — ${current.length} cells captured, 0 baselines\n`)
  console.log('  ⚠ COLD START — no baseline set.\n')
  console.log(`  ${current.length} cells were captured and none could be compared.`)
  console.log('  This is not a pass. Nothing was established, so nothing is being claimed.\n')
  console.log('  Baselines are valid on ubuntu-latest only, so they are generated by a workflow')
  console.log('  rather than locally:\n')
  console.log('      gh workflow run visual.yml -f accept-baselines=true\n')
  console.log('  That job captures, accepts, then re-captures ten times and refuses to open the')
  console.log('  pull request if any cell drifted.\n')
  console.log('  Judgment is skipped: 239 cells judged against no baseline costs real money and')
  console.log('  characterises nothing.')

  if (process.env.GITHUB_STEP_SUMMARY) {
    appendFileSync(process.env.GITHUB_STEP_SUMMARY, `### Visual comparison\n\n⚠️ ${message}\n`)
  }
  console.log(`\n::warning title=Visual baselines not established::${message}`)
  process.exit(0)
}
